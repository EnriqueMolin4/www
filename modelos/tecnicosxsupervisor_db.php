...
class Usuarios implements IConnections {
	...
	private function execute_sel() {
		try {
			...
			self::$logger->error ("File: tecnicosxsupervisor_db.php;	Method Name: execute_sel();	Functionality: Select Warehouses;	Log:" . $e->getMessage () );
		}
	}
	private function execute_ins($prepareStatement, $arrayString) {
		try {
			...
			self::$logger->error ("File: tecnicosxsupervisor_db.php;	Method Name: execute_ins();	Functionality: Insert/Update ProdReceival;	Log:" . $prepareStatement . " " . $e->getMessage () );
		}
	}
	...
	function getEstados() {
        ...
        try {
            ...
            self::$logger->error ("File: tecnicosxsupervisor_db.php;	Method Name: getEstados();	Functionality: Get Products price From PriceLists;	Log:" . $e->getMessage () );
        }
    }
    ...
    function getTable($params,$total) {
		...
		if( !empty($params['search']['value'])  &&  $total) {   
			...
			$where .=" OR du.telefono LIKE '%".$params['search']['value']."%'  )";

		}


		$sql = "select c.id,du.nombre,du.apellidos,du.email,du.telefono from cuentas c,detalle_usuarios du
				where c.id = du.cuenta_id
                and c.tipo_user = 3
                and c.territorial = $territorial
				$where 
				$order
				$filter ";
		...
		try {
			...
			self::$logger->error ("File: tecnicosxsupervisor_db.php;	Method Name: getTable();	Functionality: Get Table;	Log:".$sql . $e->getMessage () );
		}
	}
    ...
    function getEvento($id) {
        ...
        try {
            ...
            self::$logger->error ("File: tecnicosxsupervisor_db.php;	Method Name: getEvento();	Functionality: Get Products price From PriceLists;	Log:" . $e->getMessage () );
        }
	}
	...
	function getImagenesODT($id) {
        ...
        try {
            ...
            self::$logger->error ("File: tecnicosxsupervisor_db.php;	Method Name: getImagenesODT();	Functionality: Get Products price From PriceLists;	Log:" . $e->getMessage () );
        }
	}
	...
	function getHistoriaODT($id,$user) {
        ...
        try {
            ...
            self::$logger->error ("File: tecnicosxsupervisor_db.php;	Method Name: getHistoriaODT();	Functionality: Get Products price From PriceLists;	Log:" . $e->getMessage () );
        }
	}

	function getSupervisores() {
		//$sql = "select id,user from cuentas  where tipo_user in (1,11) ";
		$sql = "select c.territorial id, CONCAT(du.nombre,du.apellidos) nombre from cuentas c,detalle_usuarios du
                where c.id = du.cuenta_id
        ...
        try {
            ...
            self::$logger->error ("File: tecnicosxsupervisor_db.php;	Method Name: getSupervisores();	Functionality: Get Products price From PriceLists;	Log:" . $e->getMessage () );
        }
	}
    ...
    function buscarTecnico($search) {
		...
		try {
			...
			self::$logger->error ("File: tecnicosxsupervisor_db.php;	Method Name: buscarTecnico();	Functionality: Search Products;	Log:". $sql . $e->getMessage () );
		}
	}
...
if($module == 'addTecnico') {

    $prepareStatement = "UPDATE `cuentas` SET `territorial`=?  WHERE `id`=? ; ";
    $arrayString = array (
            $params['territorial'],
            $params['tecnicoid']
    );
