...
class Api implements IConnections {
	...
	function getTraspasosTecnico($tecnico) {
			...
			ON traspaso.no_guia = traspasoNoActivo.no_guia WHERE traspasoNoActivo.total is null ";
	...
	function getCamposObligatoriosServicio($servicioid,$cvebanco) {
		$sql = "SELECT * FROM campos_obligatorios  WHERE servicio_id = ? and cve_banco=?  ";
	...
	function getServicioIdOdt($odt) {
		...
		$sql = " SELECT eventos.tipo_servicio,eventos.servicio subservicio,eventos.odt,tipo_servicio.tipo_terminal,eventos.estatus_servicio,eventos.estatus 
	...
	function getInicioDia($tecnico) {
		$hoy = date("Y-m-d");

		$sql = "SELECT * FROM iniciodia  where tecnico=? AND primer_servicio=? ";

		try {
			$stmt = self::$connection->prepare ($sql );
			$stmt->execute (array($tecnico,$hoy));
			return $stmt->fetch ( PDO::FETCH_ASSOC );
		} catch ( PDOException $e ) {
			self::$logger->api ("File: api_db.php;	Method Name: getInventarioTecnico();	Functionality: GET CveBanco;	Log: ".  $e->getMessage () );
		}
